...
function makeFakeRuleEvaluator(result, requirementCoverage = []) {
  return {
...
function makeFakeLLMEvaluator(resultOrRejection, { rejects = false } = {}) {
  return {
    evaluate: rejects
      ? jest.fn().mockRejectedValue(resultOrRejection)
      : jest.fn().mockResolvedValue(resultOrRejection),
  };
}
...
describe("CompositeEvaluator - A. Rule + LLM both succeed", () => {
  test("preserves structural, heuristic, aiInsights, and summary; llmAvailable is true", async () => {
    const ruleResult = {
      structural: [
        {
          message: 'Class "ParkingLot" has no listed responsibilities.',
          severity: "info",
        },
      ],
      heuristic: [
        {
          message: "Spot allocation is not clearly addressed.",
          severity: "info",
        },
      ],
    };
    const llmResult = {
      aiInsights: [
        { message: "Consider extracting allocation logic into its own class." },
      ],
    ...
    const requirementCoverage = [
      {
        requirement: "The system should track entry and exit.",
        covered: true,
        coveredBy: ["ParkingLot"],
      },
    ];

    const ruleEvaluator = makeFakeRuleEvaluator(
      ruleResult,
      requirementCoverage,
    );
...
describe("CompositeEvaluator - B. LLM fails with LLMEvaluatorError", () => {
  test("preserves deterministic findings, empties aiInsights, sets llmAvailable false, provides fallback summary, and does not throw", async () => {
    ...
    const llmEvaluator = makeFakeLLMEvaluator(
      new LLMEvaluatorError("LLM evaluation timed out."),
      { rejects: true },
    );
    const composite = createCompositeEvaluator({ ruleEvaluator, llmEvaluator });
    ...
    expect(result.requirementCoverage).toEqual(requirementCoverage);
  });
});
...
describe("CompositeEvaluator - C. RuleEvaluator fails", () => {
  test("throws a CompositeEvaluatorError and never calls the LLM evaluator", async () => {
    ...
    await expect(composite.evaluate(submission, problem)).rejects.toThrow(
      CompositeEvaluatorError,
    );
  ...
  test("also handles a rejected (async) RuleEvaluator failure the same way", async () => {
    const ruleEvaluator = {
      evaluate: jest.fn().mockRejectedValue(new Error("db-ish failure")),
    };
    const llmEvaluator = { evaluate: jest.fn() };
    const composite = createCompositeEvaluator({ ruleEvaluator, llmEvaluator });
...
describe("CompositeEvaluator - D. Dependency injection", () => {
  ...
  test("swapping in different fake evaluators changes the output — proves no hardcoded concrete evaluator is used", async () => {
    const ruleEvaluatorA = makeFakeRuleEvaluator({
      structural: [{ message: "A", severity: "info" }],
      heuristic: [],
    });
    const llmEvaluatorA = makeFakeLLMEvaluator({
      aiInsights: [],
      summary: "A summary",
    });
    const compositeA = createCompositeEvaluator({
      ruleEvaluator: ruleEvaluatorA,
      llmEvaluator: llmEvaluatorA,
    });
    const resultA = await compositeA.evaluate(submission, problem);

    const ruleEvaluatorB = makeFakeRuleEvaluator({
      structural: [{ message: "B", severity: "warning" }],
      heuristic: [],
    });
    const llmEvaluatorB = makeFakeLLMEvaluator({
      aiInsights: [],
      summary: "B summary",
    });
    const compositeB = createCompositeEvaluator({
      ruleEvaluator: ruleEvaluatorB,
      llmEvaluator: llmEvaluatorB,
    });
...
describe("CompositeEvaluator - E. Deterministic findings are passed into LLMEvaluator", () => {
  test("llmEvaluator.evaluate receives structural + heuristic findings from ruleEvaluator via context.deterministicFindings", async () => {
   ...
   test("includes requirement coverage in the final feedback report", async () => {
    const ruleResult = {
      structural: [],
      heuristic: [],
    };
    ...
    const requirementCoverage = [
      ...
      {
        requirement: "The system should support different vehicle types.",
        covered: false,
        coveredBy: [],
      },
    ];
    ...
    expect(result.requirementCoverage).toEqual(requirementCoverage);
    expect(ruleEvaluator.getRequirementCoverage).toHaveBeenCalledWith(
      submission,
      problem,
    );
  });
